#include "../headers/sBlock.hpp"
#include <iostream>

//PRIVATE
std::vector<sf::RectangleShape> SBlock::createShape(std::vector<sf::RectangleShape> vector_shape, sf::Color color){
    for (int i=0; i<4; i++){
...
void SBlock::turn(std::vector<std::vector<int>> completed_square){
    sf::Vector2f vector = vector_square[1].getPosition();
    sf::Vector2f vector_unit_square = getPositionSquare(vector_square[2]);
    if (way == 0 || way ==2){
        if (vector_unit_square.y -1 >= 0 && vector_unit_square.x +1 < NUMBER_SQUARE_LENGTH && vector_unit_square.y +1 < NUMBER_SQUARE_HEIGHT){
